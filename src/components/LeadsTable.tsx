...
const LeadsTable: React.FC<LeadsTableProps> = ({ leads, onApprove, onReject }) => {
  ...
  const extraKeys = allKeys.filter(k => !coreKeys.includes(k) && !['id', 'user_id', 'referral_code', 'status'].includes(k));
              ...
              <td style={{ padding: '1rem', textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                  {lead.status === 'approved' ? (
                    <span style={{ color: '#10b981', fontWeight: '700', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
